...
CREATE PROCEDURE [dbo].[proc_awr_waits_D_S](@p_host nvarchar(50),@p_begin_snap int, @p_end_snap int, @p_top_count int, @p_order_by nvarchar(40), @p_wait_class nvarchar(40) = null)  with encryption
AS
BEGIN
	SET NOCOUNT ON
	...
	set @sqlstatement='select  @v_order_by=metric_base from ' + @p_host + '..dba_hist_control where metric_code='''+@p_order_by +''' and procname=''proc_awr_waits''
	if  NULLIF(@v_order_by, '''') IS NULL
	BEGIN
		print(''¿Tendras que ordenar por algo que exista no? Tipo de ORDER BY no valido. Saliendo...'')
		Return
	END'
	exec sp_executesql @sqlstatement,N'@v_order_by nvarchar(60) OUTPUT', @v_order_by OUTPUT
	...
	BEGIN		
		set @sqlstatement =	'DECLARE c_snap_query CURSOR FOR
			...
			from(
				...
				, dm.wait_time_ms
				, dm.max_wait_time_ms
				from ' + @p_host + '..dba_hist_dm_os_wait_stats dm
					join ' + @p_host + '..dba_hist_snapshot s
						on dm.snap_id=s.snap_id
					left outer join ' + @p_host + '..dba_hist_wait_class c
